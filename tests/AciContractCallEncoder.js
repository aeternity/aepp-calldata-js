...
test('Number of arguments validation', t => {
    ...
    t.throws(
        () => encoder.encodeCall(CONTRACT, 'test_bool', [true]),
        { name: 'EncoderError' }
    )

    t.throws(
        () => encoder.encodeCall(CONTRACT, 'test_bool', [true, true, false]),
        { name: 'EncoderError' }
    )
})
...
test('Decode implicit init (void) result', t => {
    t.plan(1)
    t.is(
        encoder.decodeResult(CONTRACT, 'init', 'cb_Xfbg4g=='),
        undefined
    )
})
...
test('Decode error result', t => {
    ...
    t.throws(
        () => encoder.decodeResult(CONTRACT, 'error', 'err_abc', 'error'),
        { name: 'FateTypeError' }
    )
})
...
test('Decode revert result', t => {
    ...
    const revert = encoder.decodeResult(CONTRACT, 'error', 'cb_OXJlcXVpcmUgZmFpbGVkarP9mg==', 'revert')
...
test('Decode unknown result', t => {
    ...
    t.throws(
        () => encoder.decodeResult(CONTRACT, 'error', '123', 'unknown'),
        { name: 'EncoderError' }
    )
})
