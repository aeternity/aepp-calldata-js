...
test('Number of arguments validation', (t) => {
    ...
    t.throws(() => encoder.encode(CONTRACT, 'test_bool', [true]), { name: 'EncoderError' })

    t.throws(() => encoder.encode(CONTRACT, 'test_bool', [true, true, false]), {
        name: 'EncoderError',
    })
})
...
test('Encode hash arguments', (t) => {
    t.plan(1)
    const encoded = encoder.encode(CONTRACT, 'test_hash', [
...
test('Ensures hash in a proper format', (t) => {
    t.plan(1)
    t.throws(() => encoder.encode(CONTRACT, 'test_hash', [{}]), {
        message:
...
test('Encode signature arguments', (t) => {
    t.plan(1)
    const encoded = encoder.encode(CONTRACT, 'test_signature', [
...
test('Encode account address arguments', (t) => {
    ...
    t.throws(() => encoder.encode(CONTRACT, 'test_account_address', ['test-string']), {
...
test('Encode contract address arguments', (t) => {
    t.plan(1)
    t.is(
        encoder.encode(CONTRACT, 'test_contract_address', [
...
test('Encode oracle address arguments', (t) => {
    t.plan(1)
    t.is(
        encoder.encode(CONTRACT, 'test_oracle_address', [
...
test('Encode oracle query address arguments', (t) => {
    t.plan(1)
    t.is(
        encoder.encode(CONTRACT, 'test_oracle_query_address', [
...
test('Ensures list is array', (t) => {
    t.plan(1)
    t.throws(() => encoder.encode(CONTRACT, 'test_list', ['test-string']), {
...
test('Encode nested list arguments', (t) => {
    t.plan(1)
    const encoded = encoder.encode(CONTRACT, 'test_nested_list', [
        [
            [1, 2],
            [3, 4],
            [5, 6],
...
test('Encode map arguments with sorted keys', (t) => {
    const encoded = encoder.encode(CONTRACT, 'test_string_map', [
        [
            ['fo', 'a'],
            ['s', 'a'],
        ],
    ...
    t.is(
        encoded,
        'cb_KxFFPju5Gy8CBXMFYQlmbwVhMOIQaw==',
        'test_string_map({["fo"] = "a", ["s"] = "a"})'
    )

    const encoded2 = encoder.encode(CONTRACT, 'test_string_map', [
        [
            // keep the order
...
test('Ensures map is map, array, object', (t) => {
    t.plan(1)
    t.throws(() => encoder.encode(CONTRACT, 'test_simple_map', ['test-string']), {
        name: 'FateTypeError',
...
test('Ensures tuple is array', (t) => {
    t.plan(1)
    t.throws(() => encoder.encode(CONTRACT, 'test_tuple', ['test-string']), {
...
test('Encode nested tuple arguments', (t) => {
    t.plan(1)
    const encoded = encoder.encode(CONTRACT, 'test_nested_tuple', [
        [
            [true, false],
            [false, true],
        ],
...
test('Variant validation', (t) => {
    const invalidData = [0xdead, null, { One: [], Two: [] }, { Fail: 'me' }, { Yep: [1, 2, 3] }]

    t.plan(invalidData.length)

    invalidData.forEach((arg) => {
        t.throws(() => encoder.encode(CONTRACT, 'test_variants', [arg]), { name: 'FateTypeError' })
    })
})
...
test('Encode variant arguments with nested variant', (t) => {
    t.plan(2)
    const encoded1 = encoder.encode(CONTRACT, 'test_nested_variants', [
        { One: [{ RelativeTTL: [7] }] },
    ])
    t.is(
        encoded1,
        'cb_KxF1qXomG6+CAQEAG6+CAQEAGw7cNGqs',
        'test_nested_variants(One(RelativeTTL(7)))'
    )

    const encoded2 = encoder.encode(CONTRACT, 'test_nested_variants', [
        {
            Two: [
                {
                    'AENS.Name': [
...
test('Encode fancy map', (t) => {
    ...
    t.is(
        encodedMap1,
        'cb_KxETMqtuKy8Br4IAAQA/Ai8Br4IAAQEbAASjVMXT',
...
test('Encode template maze', (t) => {
    t.plan(1)
    const encoded = encoder.encode(CONTRACT, 'test_template_maze', [
        {
            Any: [
...
test('Encode records', (t) => {
    ...
    const encodedNest = encoder.encode(CONTRACT, 'test_nested_record', [
        { origin: { x: 1, y: 2 }, a: 3, b: 4 },
    ])
...
test('Encode record with address and int in arguments', (t) => {
    t.plan(1)
    const encoded = encoder.encode(CONTRACT, 'test_address_record', [
...
test('Encode AENS.pointee arguments', (t) => {
    t.plan(4)
    const encoded1 = encoder.encode(CONTRACT, 'test_pointee', [
    ...
    const encoded2 = encoder.encode(CONTRACT, 'test_pointee', [
    ...
    const encoded3 = encoder.encode(CONTRACT, 'test_pointee', [
    ...
    const encoded4 = encoder.encode(CONTRACT, 'test_pointee', [
...
test('Encode AENSv2.pointee arguments', (t) => {
    t.plan(5)
    const encoded1 = encoder.encode(CONTRACT, 'test_pointee_v2', [
    ...
    const encoded2 = encoder.encode(CONTRACT, 'test_pointee_v2', [
    ...
    const encoded3 = encoder.encode(CONTRACT, 'test_pointee_v2', [
    ...
    const encoded4 = encoder.encode(CONTRACT, 'test_pointee_v2', [
    ...
    const encoded5 = encoder.encode(CONTRACT, 'test_pointee_v2', [{ 'AENSv2.DataPt': [dataPt] }])
    t.is(
        encoded5,
...
test('Encode AENS.name arguments', (t) => {
    t.plan(1)
    const encoded1 = encoder.encode(CONTRACT, 'test_aens_name', [
        {
            'AENS.Name': [
                'ak_2gx9MEFxKvY9vMG5YnqnXWv1hCsX7rgnfvBLJS4aQurustR1rt',
                { FixedTTL: [100] },
...
test('Encode AENSv2.name arguments', (t) => {
    t.plan(1)
    const encoded1 = encoder.encode(CONTRACT, 'test_aens_name_v2', [
        {
            'AENSv2.Name': [
                'ak_2gx9MEFxKvY9vMG5YnqnXWv1hCsX7rgnfvBLJS4aQurustR1rt',
                { FixedTTL: [100] },
...
test('Encode Chain.ga_meta_tx arguments', (t) => {
    t.plan(1)
    const encoded1 = encoder.encode(CONTRACT, 'test_ga_meta_tx', [
...
test('Encode Chain.paying_for_tx arguments', (t) => {
    t.plan(1)
    const encoded1 = encoder.encode(CONTRACT, 'test_paying_for_tx', [
...
test('Encode Chain.base_tx arguments', (t) => {
    t.plan(1)
    const encoded1 = encoder.encode(CONTRACT, 'test_base_tx', [
...
test('Validate Set.set arguments', (t) => {
    t.plan(1)
    t.throws(() => encoder.encode(CONTRACT, 'test_set', ['test-string']), {
        name: 'FateTypeError',
